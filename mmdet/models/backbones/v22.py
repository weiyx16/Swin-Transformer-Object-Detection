...
from mmcv_custom import load_checkpoint_swin
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0.,
                 pretrained_window_size=[0, 0], cpb_sigmoid=False, cpb_scale=1.0):

        super().__init__()
        self.dim = dim
        self.window_size = window_size  # Wh, Ww
        self.pretrained_window_size = pretrained_window_size  # Wh, Ww
        self.num_heads = num_heads
        self.cpb_sigmoid = cpb_sigmoid
        self.cpb_scale = cpb_scale

        self.logit_scale = nn.Parameter(torch.log(10 * torch.ones((num_heads, 1, 1))), requires_grad=True)
        # mlp to generate table of relative position bias
        self.rpe_mlp = nn.Sequential(nn.Linear(2, 512, bias=True),
        ...
        relative_coords_table *= self.cpb_scale  # normalize to -8, 8
        relative_coords_table = torch.sign(relative_coords_table) * torch.log2(
            torch.abs(relative_coords_table) + 1.0) / np.log2(self.cpb_scale)
    ...
    def extra_repr(self) -> str:
        return f'dim={self.dim}, window_size={self.window_size}, pretrained_window_size={self.pretrained_window_size} ' \
               f'num_heads={self.num_heads}, cpb_sigmoid={self.cpb_sigmoid}, cpb_scale={self.cpb_scale}'
...
class GlobalAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        relative_position_bias_table = self.rpe_mlp(relative_coords_table).view(-1, self.num_heads)
        ...
        if self.cpb_sigmoid:
            relative_position_bias = 16 * torch.sigmoid(relative_position_bias)
        ####################################################################################################
        # coords_h = torch.arange(0, H, dtype=torch.float32)
        # coords_w = torch.arange(0, W, dtype=torch.float32)
        # meshgrid = torch.meshgrid(coords_h, coords_w)
        # coords = torch.stack(meshgrid, -1)  # H,W,2
        # coords_flatten = coords.view(N, 2)  # N,2
        # relative_coords = coords_flatten[:, None, :] - coords_flatten[None, :, :]  # N,N,2
        # if self.pretrained_window_size[0] > 0:
        #     relative_coords[:, :, 0] /= (self.pretrained_window_size[0] - 1)
        #     relative_coords[:, :, 1] /= (self.pretrained_window_size[1] - 1)
        # else:
        #     relative_coords[:, :, 0] /= (H - 1)
        #     relative_coords[:, :, 1] /= (W - 1)
        # relative_coords *= self.cpb_scale  # normalize to -8, 8
        # relative_coords = torch.sign(relative_coords) * torch.log2(
        #     torch.abs(relative_coords) + 1.0) / np.log2(self.cpb_scale)  # N,N,2
        # relative_position_bias = self.rpe_mlp(relative_coords.to(attn.device)).view(N, N, self.num_heads)
    ...
    def extra_repr(self) -> str:
        return f'dim={self.dim}, pretrained_window_size={self.pretrained_window_size} ' \
               f'num_heads={self.num_heads}, cpb_sigmoid={self.cpb_sigmoid}, cpb_scale={self.cpb_scale}'
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
                 ...
                 cpb_sigmoid=False, cpb_scale=1.0):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        self.attn = WindowAttention(
            ...
            cpb_sigmoid=cpb_sigmoid, cpb_scale=cpb_scale)
...
class BasicLayer(nn.Module):
    ...
    Args:
        ...
        num_heads (int): Number of attention heads.
        window_size list[int]: Local window size.
        mlp_ratio (float): Ratio of mlp hidden dim to embedding dim.
        qkv_bias (bool, optional): If True, add a learnable bias to query, key, value. Default: True
        qk_scale (float | None, optional): Override default qk scale of head_dim ** -0.5 if set.
        drop (float, optional): Dropout rate. Default: 0.0
    ...
    def __init__(self,
        ...
        if len(window_size) == 1:
            self.window_size = window_size * depth
        elif len(window_size) == depth:
            self.window_size = window_size
        else:
            raise NotImplementedError

        if len(pretrained_window_size) == 1:
            self.pretrained_window_size = pretrained_window_size * depth
        elif len(pretrained_window_size) == depth:
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim,
                                 ...
                                 shift_size=0 if ((i % 2 == 0) or not use_shift) else self.window_size[i] // 2,
                                 mlp_ratio=mlp_ratio,
                                 qkv_bias=qkv_bias,
                                 drop=drop,
                                 attn_drop=attn_drop,
                                 ...
                                 pretrained_window_size=self.pretrained_window_size[i],
                                 cpb_sigmoid=cpb_sigmoid,
                                 cpb_scale=cpb_scale) if i not in global_blocks else
            SwinTransformerGlobalBlock(dim=dim,
                                       num_heads=num_heads,
                                       mlp_ratio=mlp_ratio,
                                       qkv_bias=qkv_bias,
                                       drop=drop,
    ...
    def forward(self, x, H, W):
        ...
        Args:
            ...
            H, W: Spatial resolution of the input feature.
        """
        unique_window_sizes = np.unique(self.window_size)
        attn_mask_dict = {}
        for window_size in unique_window_sizes:
            ...
            attn_mask_dict[window_size] = attn_mask

        for idx, blk in enumerate(self.blocks):
            blk.H, blk.W = H, W
            cur_attn_mask = attn_mask_dict[self.window_size[idx]]
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x, cur_attn_mask)
            else:
                x = blk(x, cur_attn_mask)
...
class SwinTransformerV2AltWin(nn.Module):
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            ...
            num_features.append(int(embed_dim * 2 ** i_layer))

        self.num_features = num_features

        # add a norm layer for each output
        for i_layer in norm_indices:
    ...
    def train(self, mode=True):
        ...
        super(SwinTransformerV2AltWin, self).train(mode)
        self._freeze_stages()